...
AI plugs into spaced repetition at several points — not only to generate cards, but to build, enrich, and maintain decks and to support review itself.

- **Creation from sources** — turn PDFs, articles, or textbooks into Q&A cards, clozes, distractors, and alternate phrasings so recall is not tied to one exact wording.
- **Richer cards** — add examples, mnemonics, images, or aligned audio (including sentence-level playback for language study).
...
- **Help on a miss** — generate a short explanation or follow-up example when you get something wrong, instead of showing only a static back-of-card answer.
...
Rete uses **new scheduling algorithms** built on the same spacing science, designed specifically around those gaps:
...
Each card has a due date. You focus on answering; Rete handles the timing.
